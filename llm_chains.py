...
def create_llm(model_path = config['model_path']['large'],
...
# We can't use our normal chain if you want to chat with the PDFs
# So, a toggle  is used to activate the PDF chat chain
class pdfChatChain:
    ...
    def __init__(self, chat_history):
        ...
        self.llm_chain = load_retrieval_chain(llm, 
                                              self.memory, 
                                              self.vector_db)
...
class chatChain:
    ...
    def __init__(self, chat_history):
        ...
        llm = create_llm()
        chat_prompt = create_prompt_from_template(memory_prompt_template)
        self.llm_chain = create_llm_chain(llm, 
                                          chat_prompt, 
                                          self.memory)

    def run(self, user_input):
        print("Normal chat chain is running...")
